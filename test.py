...
def test_batched_matmul():
    ...
    a_data = np.random.randn(B, N, M, K)
    b_data = np.random.randn(B, N, K, P)
    ...
    # 数值梯度检查
    def f_a(x):
        return np.matmul(x, b_data).sum()

    def f_b(x):
        return np.matmul(a_data, x).sum()

    grad_a_numeric = numerical_grad(f_a, a_data)
    grad_b_numeric = numerical_grad(f_b, b_data)

    # 比较误差
    err_a = np.max(np.abs(grad_a_numeric - a.grad))
    err_b = np.max(np.abs(grad_b_numeric - b.grad))
